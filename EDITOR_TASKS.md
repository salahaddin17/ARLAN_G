...
9. **Play** (Alt+P). Ожидается: две базы-коробки (teal/orange), жетоны юнитов,
   грузовики поехали к складам, сверху HUD с припасами/энергией.
10. Проверь управление: рамка ЛКМ, ПКМ приказы, A — атака-движение, S — стоп,
    H — к базе, колесо — зум, Q/W/E/R/T — контекстные действия,
    Ctrl+1..9 — группы. ВАЖНО: в PIE Ctrl+цифра может перехватываться
    редактором — тогда запускай Standalone Game (кнопка ▸ рядом с Play →
    Standalone Game).
